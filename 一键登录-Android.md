...
| strictcheck   |  2   | string |  必选  | 验证源ip合法性，填写”1”，统一认证会校验sourceid与出口ip对应关系（ip 地址的配置方法请参考接入流程文档) |
| ssotosourceid |  2   | string |  可选  | 单点登录时使用，填写被登录业务的sourceid（单点登录功能暂未开放）     |
...
| fromsourceid   | 2    | string | 可选   | 来源sourceid（即签发token sourceid）（用于单点登录，目前单点登录功能暂未开放） |
